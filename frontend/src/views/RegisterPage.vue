...
// Simple controlled inputs for registration
const name = ref('')
const email = ref('')
const password = ref('')

// Create a new account then redirect to next or /items
  ...
  <div class="min-h-screen flex items-center justify-center bg-slate-950 px-4">
    ...
    >
      <header class="flex items-start justify-between gap-3 mb-4">
        <div>
          <h1 class="text-xl sm:text-2xl font-semibold text-slate-50">Create account</h1>
          <p class="text-xs sm:text-sm text-slate-400 mt-1">
            Register a new user and get a JWT session cookie.
          </p>
        </div>
        <RouterLink
          to="/login"
      ...
      <form class="space-y-3" @submit.prevent="onSubmit">
        <label class="flex flex-col gap-1 text-xs sm:text-sm text-slate-200">
          ...
          />
          <span v-if="auth.fieldErrors.name" class="text-[11px] text-rose-300">
